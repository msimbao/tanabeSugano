...
d3.json("https://raw.githubusercontent.com/ricardo-ayres/pynabe-sugano/master/diagrams/d3.json"
).then(function(table) {
...
function makeplot() {
    Plotly.d3.csv("https://raw.githubusercontent.com/msimbao/tanabeTsugano/master/diagrams/csv/d3.csv", function(data){ processData(data) } );
};
...
console.log(Object.keys(allRows[0]).length - 1)
    for (var j=0; j<(Object.keys(allRows[0]).length - 1); j++){
        ...
        if (name ==  "4T1g" || name ==  "4T1gF" || name ==  "4T2g" || name ==  "4A2g")
        var result = {
            x: x,
            y: y,
            type: 'scatter',
...
function makePlotly( data, y, standard_deviation ){
    ...
    Plotly.newPlot('myDiv', data, layout, {
      ...
      {title: 'Tanabe Sugano Diagrams d3'},
     );
  };
    makeplot();
